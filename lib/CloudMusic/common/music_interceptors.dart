...
class MusicApiInterceptors extends InterceptorsWrapper {
  ...
  void onRequest(
    ...
    options.headers[HttpHeaders.userAgentHeader] = getUserAgent(
      options.extra['userAgent'],
    );
  ...
  Future<List<Cookie>> getCookies({Uri? host}) async {
    ...
    return (globalCookieJar!).loadForRequest(host);
  }
  ...
  Future<void> deleteAllCookie() async {
    await (globalCookieJar! as PersistCookieJar).deleteAll();
  }

  void _handleLinuxForward(RequestOptions options) {
  ...
  void _handleWeApi(RequestOptions options) {
    ...
    if (csrfToken.isNotEmpty) {
      // map可能为 String,Int>类型的，默认转换为 String,dynamic>
  ...
  void _handleEApi(
    ...
    if (type == EncryptType.eApi) {
      ...
      options.data = options.data.entries
          .map((e) => '${e.key}=${e.value}')
          .join('&');
    }
  }
